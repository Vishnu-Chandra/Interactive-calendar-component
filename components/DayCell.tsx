...
export default function DayCell({
	...
	if (day === null) {
		return (
			<div
				className="flex h-10 w-full items-center justify-center sm:h-10"
				aria-hidden="true"
			/>
		);
	}
	...
	const todayClass = isToday && !hasRangeState ? "" : "";
	...
	return (
		<button
			type="button"
			data-day-cell="true"
			className={`relative flex h-10 w-full items-center justify-center border-0 text-[0.74rem] leading-none transition-all duration-200 hover:scale-105 active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-[1px] focus-visible:outline-[#7cbdf0] sm:h-10 sm:text-[0.78rem] ${baseAppearanceClass} ${stateClass} ${rangeShapeClass} ${todayClass}`}
			style={{
			...
			aria-pressed={isStart || isEnd || isInRange || isPendingRange}
			aria-label={
				holidayLabel
		...
		>
			...
			{holidayLabel ? (
				<span
					className="absolute right-1 top-1 h-2 w-2 rounded-full bg-rose-500 ring-1 ring-white"
					aria-hidden="true"
				/>
			) : null}
		</button>
